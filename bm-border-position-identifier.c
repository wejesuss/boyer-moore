...
void print_borders(char **prefixes, char **suffixes, int *test, int removed, int patlen)
{
    int i = 0;
    int blen = 0;
    test[removed] = patlen + removed;
    test[patlen + removed] = patlen + removed + 1;
    while (i < patlen)
    {
        if (strcmp(prefixes[i], suffixes[i]) == 0)
        {
            if (strlen(suffixes[i]) > blen)
            {
                blen = strlen(suffixes[i]);
                test[removed] = patlen + removed - i;
            }
...
int search_for_borders(char *pat, int *test, int *removed)
{
    ...
    printf("\n");
    print_borders(prefixes, suffixes, test, *removed, patlen);

    if (patlen > 1)
    {
        (*removed)++;
        search_for_borders(&pat[1], test, removed);
    }
...
int main(void)
{
    ...
    int patlen = strlen(pat);
    int test[patlen + 1];
    memset(test, 0, sizeof(int) * (patlen + 1));
    int removed = 0;
    int error_code = 0;

    error_code = search_for_borders(pat, test, &removed);
